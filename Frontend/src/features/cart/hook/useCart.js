...
const useCart = () => {
    ...
    async function handleViewCart() {
        try {
            ...
            // Aggregate returns: { data: [{ _id, totalPrice, currency, items }] }
    ...
    async function handleRemoveFromCart(itemId) {
        try {
            ...
            // After remove, re-fetch to get fresh aggregate data
            await handleViewCart();
    ...
    async function handleUpdateQuantity(itemId, quantity) {
        try {
            ...
            // After update, re-fetch to get fresh aggregate data
            await handleViewCart();
        } catch (err) {
